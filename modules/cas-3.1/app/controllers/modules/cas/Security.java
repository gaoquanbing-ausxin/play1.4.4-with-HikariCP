...
public class Security {
    ...
    public static Boolean isAuthorizedURL(String resourceURL, String requestMethodType) {

        System.out.println("invoke url is : " + resourceURL);
        if(resourceURL == null) {
            return Boolean.FALSE;
        }
        Object object = Cache.get(resourceURL);
        String[] sysInterfaceArray = null;

        if(object == null) {
            return Boolean.FALSE;
        } else {
            sysInterfaceArray = object.toString().split(";");
        }

        Boolean isNeedValidation = Boolean.valueOf(sysInterfaceArray[0]);
        String methodType = sysInterfaceArray[2];

        if (isNeedValidation) {

            if (requestMethodType.equals(methodType) || methodType.equals("ALL")) {
                return Scope.Session.current().get(String.valueOf(resourceURL.hashCode())) != null;
            }
            else {
                return Boolean.FALSE;
            }
